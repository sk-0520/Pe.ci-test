				...
				'logs': [
					{
						...
						'comments': [
							'ツールバー設定からランチャーのメニューボタンを表示するかどうか設定可能にしました','メニューボタン非表示状態でメニューを表示するには「マウス中央ボタン」、「Shift + 右クリック」のどちらかの操作が必要です'
						]
					},
					{
					...
					},
					{
						...
						'comments': [
							'F1キーの誤入力を防ぐ機能です','本機能が有効な時にF1を入力したい場合は F1 を二回入力してください','設定箇所: 設定→本体設定→システム環境→F1キーを抑制'
						]
					}
				]
			},
				...
				'logs': [
					{
						...
						'comments': [
							'ツールバーのコンテキストメニューからはみ出したアイテムを選択できます','ただしこの場合は通常実行の挙動に限定されます'
						]
					},
					{
						...
						'comments': [
							'ランチャーアイテム側で削除したアイテムがグループ配下側で内部的に保持されたまま不可視だったことが原因なのでこれを可視化した','ついでに #456 対応'
						]
					},
					{
				...
				'logs': [
					{
						...
						'comments': [
							'コマンド入力保管対象を設定することで切り替えます(デフォルトで有効)','入力補完時に有効設定アイテムのみ列挙し、入力が完全一致する場合は設定が無効でも列挙されます','0.77.0 以下からのアップデートは全アイテムがデフォルト値に強制されます'
						]
					},
					{
				...
				'logs': [
					{
						...
						'comments': [
							'端末が一定時間アイドル状態であれば各データをアーカイブするよう変更','設定値はApp.config(PeMain.exe.config)','変更に伴いクリップボードのアーカイブ間隔を変更','アイドル監視時間: 8分','アイドル判定時間: 70分','クリップボード閾値: 3時間'
						]
					}
				]
			},
			{
				...
				'logs': [
					...
					},
					{
						...
						'comments': [
							'例外捕まえただけの暫定対応','原因調査してないので今後やっていく'
						]
					}
				]
			},
				...
				'logs': [
					{
						...
						'comments': [
							'調査結果としては設定ファイルのバックアップ、特にクリップボード全件保存の場合に各データファイルのバックアップに時間がかかっていた','クリップボード・ノート・テンプレートの実データが閾値に該当するものをアーカイブした状態で保持するように変更','クリップボード閾値: 更新が 3 日前で 256KB 以下','ノート閾値: 更新が 7 日前で 1KB 以下','テンプレート閾値: 更新が 10 日前で 4KB 以下','閾値は App.config(PeMain.config) で定義されてるので内容については #364 を参照してください','Pe 終了時にバックアップ→本処理の流れで実施されるためアップデート後の二回目終了時に効力が現れます','デバッグ版ではうまくいったよ！デバッグ版ではね！'
						]
					},
					{
					...
					},
					{
						...
						'comments': [
							'内部的に掴んでいるファイルをさらに掴もうとしていました','一部ややこしい問題もありました','根本的に処理変えたところもありました','おちこんだりもしたけれど、私はげんきです'
						]
					}
				]
			},
		...
		'contents': [
			{
				...
				'logs': [
					...
					{
						'class': 'compatibility',
						'subject': '#415の影響によりユーザー設定ディレクトリ(標準だと %APPDATA%\Pe\settings あたり)の *.tmp ファイルが削除対象となりました',
						'comments': [
				...
				'logs': [
					{
						...
						'comments': [
							'#393, #355でどうにもうまくいかなかった原因','なんかロジック的には正しかったけど内部使用しているパラメータの扱いがミスってた'
						]
					},
					{
				...
				'logs': [
					{
						...
						'comments': [
							'通知領域コンテキストメニュー→情報→ヘルプ','ひっさしぶりに生のHTML/CSS書いた','読み込み時の細かい処理は追々調整する'
						]
					},
					{
						...
						'comments': [
							'設定→本体設定→操作通知','Windows 10 で出まくるの鬱陶しいので初期値は「なし」に設定'
						]
					},
					{
					...
					},
					{
						...
						'comments': [
							'#355の逆襲','本実装に伴いファイルへのログ出力実装を変更したけ通常使用には無関係'
						]
					}
				]
			},
				...
				'logs': [
					{
						...
						'comments': [
							'本バージョンから .NET Framework 4.6 が必要になります','.NET Framework 4.6 は https://www.microsoft.com/ja-jp/download/details.aspx?id=48130 からダウンロードできます'
						]
					},
					{
						'subject': '本バージョンからアップデートチェックに使用するアドレスが変更となります',
						'comments': [
							'XML -> https://bitbucket.org/sk_0520/pe/downloads/update.xml','HTML(Release) -> https://bitbucket.org/sk_0520/pe/downloads/update-release.html','HTML(RC) -> https://bitbucket.org/sk_0520/pe/downloads/update-rc.html','bitbucketのダウンローダーはレスポンスに`Content-Disposition: attachment;`があるけど大丈夫だろ'
						]
					}
				]
			},
			...
			},
			{
				...
				'logs': [
					...
					},
					{
						...
						'comments': [
							'1. 設定UIでの制御ができていなかった','2. 設定補正時に強制ONになっていた'
						]
					},
					{
					...
					},
					{
						...
						'comments': [
							'アップデート確認と同じ処理方法だと思ってたら全然違ってた','みんな大好き try ... catch(Exception) で対応'
						]
					},
					{
				...
				'logs': [
					{
						...
						'subject': '#335: .NET Frameworkのバージョンを4.6に変更',
						'comments': [
							'やっとこさ nameof が使えるようになったので目についた範囲を修正','実装の移行はのんびりやっていく'
						]
					},
					{
						'subject': '#25: ヘルプファイルの記述',
				...
				'logs': [
					{
						...
						'comments': [
							'0.73.0 で .NET Framework の対応バージョンを を 4.5.1 から 4.6 に変更します','nameof! nameof! nameof!','#355の影響で0.72.0→0.73.0に先延ばし'
						]
					},
					{
					...
					},
					{
						...
						'comments': [
							'課題への記入を匿名でも行えるようにするか悩み中です','フィードバック機能も実装したし賛成・反対意見をもらえるとありがたいです'
						]
					}
				]
			},
				...
				'logs': [
					{
						...
						'comments': [
							'フィードバック等のユーザ情報収集時に使用されます','設定画面の本体設定タブで再設定が可能です','規定値ではユーザー環境のユーザー名・OS・CPU・メモリをコネコネしてMD5を算出します','ランダム生成した時間は現在時間からMD5を算出します','UI追加に伴いランチャーD&D設定はランチャータブへ移動しました'
						]
					},
					{
						...
						'comments': [
							'通知領域Peアイコンコンテキストメニュー 情報→フィードバックで入力できます','入力データの送信にはインターネット接続が必要になります','データに関しては追々ヘルプ書きます','入力データを破棄したい場合は DATA-ID をお伝えください','本対応によりレジストリ情報の一部に書き込みが行われます。Pe起動時に書き込まれ終了時に破棄されます。キーは下記になります','HKEY_CURRENT_USER\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_BROWSER_EMULATION','HKEY_CURRENT_USER\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_DOCUMENT_COMPATIBLE_MODE','過去に記入して頂いたフィードバックは破棄します','ていうか余裕なくてあんまり見れてませんでした。ごめんちゃい'
						]
					},
					{
					...
					},
					{
						...
						'comments': [
							'コマンドランチャーとランチャーアイテム一覧で実装が分かれていたのを統合しました','入力文字列の先頭1文字で検索方法が変わります','大文字: 前方一致 + 大文字小文字を区別する','小文字: 部分一致 + 大文字小文字を区別しない'
						]
					},
					{
						...
						'comments': [
							'#297での実装と環境を流用してユーザー情報を収集します','今のところ実行タイミングは起動時・セッション開始時になります(自動アップデート確認と同じタイミング)','設定→プライバシー から「ユーザー情報送信を許可」が有効になっている場合のみ送信処理が行われます','送信内容はヘルプに記載する予定ですがまだヘルプが書けていません','内容を確認したい場合はログを確認してください。要求・応答メッセージが出力されています','送信データを破棄したい場合は DATA-ID をお伝えください','本機能実装により本バージョンアップデートには使用許諾が表示されます'
						]
					},
					{
				...
				'logs': [
					{
						...
						'comments': [
							'うまくいかんし#355自体は対応やめます','#393で頑張りますん'
						]
					}
				]
			}
			...
			},
			{
				...
				'logs': [
					...
					},
					{
						...
						'subject': '#355: 終了時にログを強制出力する',
						'comments': [
							'0.70.0での実装は色々残念だった','>ログ保存位置を指定していない場合(標準動作)は %APPDATA%\logs に強制出力ログファイル(session-ending.log)が作成されていました','>>%APPDATA%直下に汎用的な名前でディレクトリを作っちゃったのでPe側では削除できません','>>>気になるのであれば削除しといてください','まぁそもそも出力失敗してるから0byteファイルなんですけどね！'
						]
					},
					{
						...
						'comments': [
							'修正の簡易さからアイコンとタイムスタンプを左寄せにした','ついでにテンプレートも同じスタイルに設定'
						]
					},
					{
					...
					},
					{
						...
						'comments': [
							'実装見ると変ではなかったけど直観的ではなかったので動作変更','範囲指定した場合、今までは一番古いものを基準としたが本バージョンから新しいものを基準とするように変更','でもまぁ#363に食われるだろうけど'
						]
					},
					{
						'revision': '43183959f9c4e3e127ed9272117770b0095e9091',
						...
						'comments': [
							'本バージョンへアップデートした際に重複判定件数が 50(0.70.0以下の規定値) であれば -1(全件) に変換されます','0.71.0から -1 が規定値になります'
						]
					}
				]
			},
				...
				'logs': [
					{
						...
						'comments': [
							'有効になるのは次回アップデート時です','これと言ってユーザー側に意味はありません'
						]
					}
				]
			}
				...
				'logs': [
					{
						...
						'comments': [
							'基本ロジックと一部データには適用','全データに設定するのは労力的にしんどいので追々適用していく'
						]
					},
					{
						...
						'subject': 'IDE0001の抑制',
						'comments': [
							'usingする名前空間をVS2013スタイルで留める','親以降の名前空間が同じ名称結構多くて完全修飾の方が分かりやすいのですよ'
						]
					},
					{
					...
					},
					{
						...
						'subject': '#355: 終了時にログを強制出力する',
						'comments': [
							'#353のため#354の下準備から#355まで実装','一番親元の作業オブジェクトに追加したのでView側で固まってたら再調査が必要なので保留とする','ステータスは課題を参照のこと'
						]
					}
				]
			}
			...
			},
			{
				...
				'logs': [
					...
					},
					{
						'subject': '#305: メモリ消費を抑える',
						'comments': [
							'使いまわせる ViewModel は再生成を抑える','意味あんのか知らんけど一部バインドを初回のみに変更'
						]
					},
					{
					...
					},
					{
						...
						'comments': [
							'左側に出すように変更','OSの利き手設定によって表示方向が右だったりするけど気にしない'
						]
					},
					{
			...
			},
			{
				...
				'logs': [
					...
					},
					{
						'subject': '#305: メモリ消費を抑える',
						'comments': [
							'焼け石に水かもだけど ViewModel を破棄した時に Model の参照を外す','ツールバーのGUI構築方法を改善'
						]
					},
					{
			...
			},
			{
				...
				'logs': [
					...
					},
					{
						...
						'comments': [
							'BooleanToVisibilityConverter くらいしかなかった','今後見つけ次第修正していく'
						]
					}
				]
			}
			...
			},
			{
				...
				'logs': [
					...
					},
					{
						...
						'comments': [
							'ヘッダ部とデータ部で管理されているノート・クリップボード・テンプレートのメモリ管理方法が改善されました','特にクリップボードの画像データによるメモリ圧迫が改善された気がします'
						]
					},
					{
				...
				'logs': [
					{
						...
						'comments': [
							'データ補正時に固定・最小化を無効にしてた','多分初期化でやりたかった内容が補正側に入ってた'
						]
					},
					{
				...
				'logs': [
					{
						...
						'comments': [
							'基本的な機能はForms版の踏襲ですがあくまで似ているだけです','今後の機能追加・保守はWPF版のみになります','本バージョンはあくまでForms→WPFへの移植で溜まっていた課題への対応は次回バージョンから頑張る','実装期間長かったー！','出張先からのリリースなのでWPF版でのアップデート試験してないけどいけるさ、大丈夫さ、気にするな'
						]
					},
					{
						'class': 'compatibility',
						...
						'comments': [
							'!注意! バグバグしてそうな本バージョンへのアップデートを見送るユーザーもいそうなので、下記引き継ぎ機能は未来バージョン数世代はサポートします','引き継ぎ処理はWPF版本体設定が存在せずForms版本体設定が存在する場合に実施されます','引き継がれる設定: 基本設定、ランチャーアイテム、グループ','引き継がれない設定: 各ノートデータ、各クリップボードデータ、各テンプレートデータ','ランチャーアイテム互換性: ディレクトリアイテムはファイルアイテムに変換されます(内部実装として地味に予約しているので要望があれば検討します)','ランチャーアイテム互換性: 組み込みアイテムは引き継ぎ対象外になります(将来的にはまたサポートしますが今は休止)'
						]
					},
					{
						'subject': 'ツールバー',
						'comments': [
							'Aero Glass を使用しなくなりました','「自動的に隠す」設定時に隠れる際のアニメーションを廃止しました(実装時に設計をミスった)','ランチャーアイテムのメニューからファイル一覧メニューがなくなりました','ランチャーアイテムのメニューからアイテム編集が行えるようになりました','コマンドアイテムもファイルアイテムのように実行できるようにしました','ALTキー押下によるアイテム並び替え機能が廃止されました(現実装だとちっと難しそうなので後回し)','ESCキー二回押下でツールバーを隠す機能は一旦廃止しました(実装忘れてた)'
						]
					},
					{
						'subject': 'ノート',
						'comments': [
							'各種設定編集をメニューからでなく一元的に操作できるようになりました','通知領域からの一覧表示メニューが表示中・非表示に分離されました'
						]
					},
					{
						'subject': 'コマンド',
						'comments': [
							'URL入力機能を廃止しました(実装忘れてた)','候補一覧にアイコンを表示しました'
						]
					},
					{
						'subject': 'クリップボード',
						'comments': [
							'データ保持方法を変更。Forms版で大きな画像ばかり取り込んだ際に落ちる不具合が解消されたと思います。思うだけで試してません','クリップボードの各種データ形式に対して取込制限設定を追加しました','取込種類・保存種類を統合しました'
						]
					},
					{
						'subject': 'テンプレート',
		...
		'contents': [
			{
				...
				'logs': [
					...
					},
					{
						...
						'comments': [
							'今後はWPF版としてリリースされる予定です','未決定ですがWPF版とForms版での設定データに互換性はありません','>変換処理実装に割く時間が無いかもなのです','>>互換性を持たせるための処理・検証よりWPF版リリースを優先してそこから発生した不具合の修正を優先したいのです','よっぽどおかしな処理があればForms版でも修正入れますが、ほぼほぼ無いです'
						]
					}
				]
			},
				...
				'logs': [
					{
						...
						'comments': [
							'作業ディレクトリが設定されている場合の挙動は0.60.0以前と変わりありません','作業ディレクトリが設定されていない場合、実行パスの親ディレクトリが作業ディレクトリとして使用されます'
						]
					}
				]
			},
				...
				'logs': [
					{
						...
						'comments': [
							'入力中に[TAB]キーを押下することにより補完を行います','ノリと勢いだけで実装したので細かい挙動は気にしないでいただきたいなぁと思ってる、と書いとけばいいって予防線の張り方'
						]
					},
					{
			...
			},
			{
				...
				'logs': [
					...
					},
					{
						...
						'comments': [
							'ゲームしようとフルスクリーンにするとツールバー状態のPeがそれを解除しようとしてゲーム進行が止まるのですよ','Crysis2の時は大丈夫だったんだけど昨日買ったCrysis3だと止まるんだよ！ だれだよこんなソース書いたやつは！'
						]
					}
				]
			}
				...
				'logs': [
					{
						...
						'comments': [
							'過去バージョンから設定を引き継ぐ場合、タグ・ファイル検索機能は無効になってます','>設定補正だるかったし否定形の設定項目作るのに気が引けたのよ','アイコン設定は実装してるけど下記事情によりリスト上のアイコン描画処理は将来実装','>>描画そのものは出来るんすよ','>>>出来るけど ComboBox の TextBox 部分のサイズ(高さ)がかなり残念なことになる','>>>>色々やってはみたけどこれだけのためにリリース伸ばすのもかったるかった','はよWPFに移りたい、しんどい'
						]
					}
				]
			},
				...
				'logs': [
					{
						...
						'comments': [
							'それっぽくは動くけど勇んで走り出したTaskの行方は誰も知らない','正直なところ白旗'
						]
					},
					{
			...
			},
			{
				...
				'logs': [
					...
					},
					{
						...
						'comments': [
							'あっかん、再現できん','例外ガン無視する','Exception 捕まえずに try { ... } catch(AccessViolationException) { ... } で自重した私をほめてください'
						]
					}
				]
			},
				...
				'logs': [
					{
						...
						'comments': [
							'Windows8で隠れないらしいので暫定的に対処','再現環境がないので何とも言えない','#182に干渉するけどまぁ、うん'
						]
					}
				]
			}
				...
				'logs': [
					{
						...
						'comments': [
							'ディスプレイ関係は切り替えやすいラップトップで作業してて分かったけど、tpscrex.exe(ThinkPadの解像度変更用ユーティリティ)を使用した場合にのみ強制的にリサイズされノートのサイズがおかしくなる','ツールバーも同じようにサイズ・位置が強制的に変更されていたがデスクトップツールバーとしてPe側でさらに強制的にリサイズしていたので表面化しなかった模様','なので発生し得る環境がかなり限定されるが修正箇所自体は全環境に恩恵があると思うのでマージした'
						]
					},
					{
				...
				'logs': [
					{
						...
						'comments': [
							'フィルタリング機能を追加しました','フィルタリング中はテンプレートアイテムの追加・移動が抑制されます'
						]
					},
					{
						...
						'comments': [
							'置き換え処理が有効であればさらにT4テンプレートエンジンを使用した置き換えを行えます','プログラム書ける人でかつ大規模なテンプレートを書かない人が対象です','T4はMono.TextTemplatingを使用しているためMS製T4と動作が違うかもです','暗黙的に <#@ template language="C#" hostspecific="true" culture="使用言語の言語コード" #> が先頭行に挿入されます','Pe側で __host(内部使用), app(IReadOnlyDictionary<string,object>) を予約します。Peの提供するデータには app[string] でアクセスしてください','将来的にはもうちっと頑張ろうと思いますがとりあえず#233実装はここまで'
						]
					},
					{
						...
						'comments': [
							'<Pe>/bat/beta.bat を実行すると現行バージョンに影響することなくβバージョンを実行することができます','beta.bat 実行時に デスクトップ/Pe-beta ディレクトリが存在しなければ現行バージョンの設定データ(デフォルトパス)を デスクトップ/Pe-beta ディレクトリにコピーします','あくまでβバージョンとして動作させるための機能ですのでリリース版で実行する意味はありません'
						]
					}
				]
			},
		...
		'contents': [
			{
				...
				'logs': [
					...
					},
					{
						...
						'comments': [
							'標準入出力関係の設定データを内部的に独立させました','元の設定項目が一つだけでロジックに影響せずUIだけが影響されるものであるため下位互換を維持するだけの価値がないのでバッサリ切りました'
						]
					}
				]
			},
				...
				'logs': [
					{
						...
						'comments': [
							'バージョンアップに失敗し、プログラムが強制終了した場合はPeを再起動してタスクトレイコンテキストメニュー → Pe情報 → アップデートを実行してみてくださいよ！','どうしようもなくアップデートできない場合は https://bitbucket.org/sk_0520/pe/downloads からダウンロードしてください。。。Vectorは公開依頼してから公開までが遅いのですよ！','もう大丈夫だ、大丈夫、これで落ちない、大丈夫大丈夫。大丈夫だから忘れよう'
						]
					}
				]
			},
		...
		'contents': [
			{
				...
				'logs': [
					...
					},
					{
						...
						'comments': [
							'本バージョンで対応したつもりですよ','アップデート用スレッドとか無関係そうなツールバーのリソース処理とか色々あったぽいのですよ','バージョンアップに失敗し、プログラムが強制終了した場合はPeを再起動して(Windowsセッション接続維持中に)タスクトレイコンテキストメニュー → Pe情報 → アップデートを実行してみてください','どうしようもなくアップデートできない場合は https://bitbucket.org/sk_0520/pe/downloads からダウンロードしてください。。。Vectorは公開依頼してから公開までが遅いのです'
						]
					}
				]
			},
				...
				'logs': [
					{
						...
						'comments': [
							'履歴数やその内容によって負荷が異なりますのでユーザー設定で対応してください','Pe 0.50.0の挙動と同じにするには値を 1 に設定してください'
						]
					},
					{
					...
					},
					{
						...
						'comments': [
							'コマンドアイテムでも標準入出力を操作できるようになりました','管理者として実行するにはきちんとファイルアイテムで登録してください','指定コマンドが実行可能か、設定パラメーターが伝搬するかはコマンドに左右されるため注意して下さい','コマンドアイテムはパラメーターが設定済みであることを期待します。その都度パラメーターを変更する用途にはファイルアイテムが適切です'
						]
					}
				]
			},
				...
				'logs': [
					{
						...
						'comments': [
							'とりあえず、とりあえず動くようにした','本対応に伴い情報ウィンドウのレイアウトが変更となりました'
						]
					},
					{
						...
						'comments': [
							'セッションを引き金とするアップデートチェックをログオン時に限定','ツールバー破棄後のウィンドウハンドルアクセスも同時に修正'
						]
					},
					{
				...
				'logs': [
					{
						...
						'comments': [
							'今のところ呼び出し自体はUIスレッドだから不具合にはなってないはず','……はず'
						]
					}
				]
			}
				...
				'logs': [
					{
						...
						'comments': [
							'画像の判定はちょっと厳しいので後回し','画像は暇な時にビット深度が固定・変動するのか調べて実装する'
						]
					}
				]
			},
				...
				'logs': [
					{
						...
						'comments': [
							'圧縮してはいるものの、データよっては保存ファイルのサイズが大きくなるので保存機能はデフォルトでは無効になっています','保存機能を有能しても保存種別が未チェックであればデータは保存されません','保存種別(特に画像)によっては保存ファイルサイズが肥大化しますので注意してください'
						]
					},
					{
					...
					},
					{
						...
						'comments': [
							'HTMLクリップボードはクリップボード取得時のURIの表示・コピーを追加しました','画像は初期状態でウィンドウサイズに合わせて表示されます','画像を原寸表示している場合に左クリックのD&Dでスクロールします'
						]
					}
				]
			},
				...
				'logs': [
					{
						...
						'comments': [
							'グループノード選択時にランチャーアイテム一覧をダブルクリックすると選択中ランチャーアイテムが追加されます','各ノードをD&Dで移動出来るようになりました'
						]
					}
				]
			},
			{
				...
				'logs': [
					...
					},
					{
						...
						'comments': [
							'全項目に適用するとアホみたいに遅くなるし現在表示項目数が取得できないので上位項目に適用する','ファイル数が多いとクッソ怪しい動作','system32なんて誰も表示しないだろうという一握の望み'
						]
					},
					{
			...
			},
			{
				...
				'logs': [
					...
					},
					{
						...
						'comments': [
							'ツールバーへファイルをD&Dで登録した際に対象ツールバー以外のグループがクリアされる','ランチャーアイテムをD&Dして並べ替えた際に他のツールバーのアイテム順序が追従しない'
						]
					},
					{
				...
				'logs': [
					{
						...
						'comments': [
							'CLIP','CLIP:NOBREAK','CLIP:HEAD','CLIP:TAIL'
						]
					}
				]
			},
			{
				...
				'logs': [
					...
					},
					{
						...
						'comments': [
							'VER-FULL -> VER','VER-NUMBER -> VER:NUMBER','VER-HASH -> VER:HASH'
						]
					},
					{
				...
				'logs': [
					{
						...
						'comments': [
							'定型文をアイテムとして保存、それをクリップボードへコピーします','置き換え処理を使用すれば実行時の年月日などを設定できます'
						]
					}
				]
			},
			...
			},
			{
				...
				'logs': [
					...
					},
					{
						...
						'comments': [
							'sbin/Updaterが戻り値を返す','CLIでのキー押せ催促を統一'
						]
					},
					{
				...
				'logs': [
					{
						...
						'comments': [
							'とりあえず現状では以下のタイミングで処理する','起動','設定保存','ホームダイアログ終了','セッション接続接続','ロック解除','システム再開'
						]
					}
				]
			},
			{
				...
				'logs': [
					...
					},
					{
						...
						'comments': [
							'アイコン取得時に一回のアクセスで取得できない場合があるので数回アクセスするように変更','通常アイコン取得時、API成功でもアイコンハンドルが取得できてない場合に後続処理を行わない'
						]
					}
				]
			},
			...
			},
			{
				...
				'logs': [
					...
					},
					{
						...
						'subject': 'アップデート処理で bin\PeUpdater.exe.config の削除処理が抜けていた'
					}
				]
			}
		]
