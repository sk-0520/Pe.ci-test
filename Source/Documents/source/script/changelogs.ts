			...
			},
			{
				...
				"logs": [
					...
					},
					{
						...
						"subject": "#673: 同一のテーマプラグインを選択している状態で本体設定を保存すると二重読み込みで死ぬ",
						"comments": [
					...
					},
					{
						...
						"class": "nuget",
						"subject": "SonarAnalyzer.CSharp 8.9.0.19135 -> 8.12.0.21095"
					}
				]
			}
		]
...
/*--------BUILD-EMBEDDED-JSON--------*/
//import foo from './changelogs-archive.json';
import { json as json }  from './changelogs-archive.json';
...
alert(changelogs.length);
alert(json.length);
Array.prototype.push.apply(changelogs, json);
alert(changelogs.length);
