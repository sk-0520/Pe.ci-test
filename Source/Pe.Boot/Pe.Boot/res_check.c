...
static rc_heap__ALLOC_STOCK_ITEM rc_heap__allocStocks[MEM_CHECK_ALLOC_STOCK_LENGTH] = { 0 };
static size_t rc_heap__allocStocksCount = 0;

#define rc_heap__debug_output(format, ...) do { \
    TCHAR rc_heap__debug_buffer[MEM_CHECK_PRINT_BUFFER_LENGTH] = { 0 }; \
    ...
    if (allocate) {
        ...
        for (size_t i = 0; i < MEM_CHECK_ALLOC_STOCK_LENGTH; i++) {
            if (!rc_heap__allocStocks[i].p) {
                rc_heap__debug_output("[MEM:+] %p %s(%zu)", p, caller_file, caller_line);
        ...
        if (!stocked) {
            rc_heap__debug_output("[MEM:STOCK:ERROR] %p %s(%zu)", p, caller_file, caller_line);
        }
    } else {
        bool exists = false;;
        for (size_t i = 0; p && i < MEM_CHECK_ALLOC_STOCK_LENGTH; i++) {
            if (rc_heap__allocStocks[i].p == p) {
                rc_heap__debug_output("[MEM:-] %p %s(%zu) - %s(%zu)", p, caller_file, caller_line, rc_heap__allocStocks[i].file, rc_heap__allocStocks[i].line);
        ...
        if (!exists) {
            rc_heap__debug_output("[MEM:NOTFOUND:ERROR] %p %s(%zu)", p, caller_file, caller_line);
        }
    }
}
