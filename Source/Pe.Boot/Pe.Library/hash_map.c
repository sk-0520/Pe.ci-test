...
static size_t get_capacity(const HASH_MAP* map)
{
...
static KEY_VALUE_PAIR RC_HEAP_FUNC(new_map_pair, const HASH_MAP* map, const TEXT* key, const void* value)
{
    KEY_VALUE_PAIR pair = {
...
static bool search_map_from_key_core(const void* needle, const void* value, void* data, void* arg)
{
    ...
    const HASH_MAP* map = (const HASH_MAP*)arg;

    return map->library.equals_map_key(needle_key, &pair->key);
}

static LINK_NODE* search_map_from_key(const LINKED_LIST* linked_list, const TEXT* needle, const HASH_MAP* map)
{
...
static LINKED_LIST* RC_HEAP_FUNC(new_items, size_t capacity, func_release_hash_map_value release_hash_map_value, const MEMORY_RESOURCE* value_memory_resource, const MEMORY_RESOURCE* map_memory_resource)
{
    LINKED_LIST* item_lists = RC_HEAP_CALL(new_memory, capacity, sizeof(LINKED_LIST), map_memory_resource);
    for (size_t i = 0; i < capacity; i++) {
        LINKED_LIST item_list = RC_HEAP_CALL(new_linked_list, sizeof(KEY_VALUE_PAIR), (void*)release_hash_map_value, compare_linked_list_value_null, release_link_item_value, value_memory_resource, map_memory_resource);
...
HASH_MAP RC_HEAP_FUNC(new_hash_map, byte_t item_size, size_t capacity, double load_factor, func_release_hash_map_value release_hash_map_value, func_calc_hash_map_hash calc_map_hash, func_equals_hash_map_key equals_map_key, const MEMORY_RESOURCE* value_memory_resource, const MEMORY_RESOURCE* map_memory_resource)
{
    ...
    HASH_MAP result = {
        ...
        .library = {
            ...
            .items = RC_HEAP_CALL(new_items, adjusted_capacity, release_hash_map_value, value_memory_resource, map_memory_resource),
            .map_memory_resource = map_memory_resource,
            .value_memory_resource = value_memory_resource,
            .release_hash_map_value = release_hash_map_value,
...
void RC_HEAP_FUNC(release_hash_map_core, HASH_MAP* map, bool value_release)
{
...
bool RC_HEAP_FUNC(release_hash_map, HASH_MAP* map)
{
    ...
    RC_HEAP_CALL(release_hash_map_core, map, true);

    return true;
}

static void RC_HEAP_FUNC(extend_map_if_over_load_factor, HASH_MAP* map)
{
    ...
    LINKED_LIST* next_items = RC_HEAP_CALL(new_items, new_capacity, map->library.release_hash_map_value, map->library.value_memory_resource, map->library.map_memory_resource);
    ...
    for (size_t i = 0; i < flat_list_length; i++) {
        ...
        RC_HEAP_CALL(add_hash_map, map, &pair->key, pair->value);
    }
...
HASH_MAP_RESULT_VALUE get_hash_map(const HASH_MAP* map, const TEXT* key)
{
    ...
    if (!node) {
        HASH_MAP_RESULT_VALUE not_found_item = {
            .value = NULL,
            .exists = false,
        };
