...
TEXT RC_HEAP_FUNC(expand_environment_variable, const TEXT* text, const MEMORY_RESOURCE* memory_resource)
{
    ...
    TCHAR* buffer = RC_HEAP_CALL(allocate_string, buffer_size, memory_resource);
    DWORD result = ExpandEnvironmentStrings(text->value, buffer, buffer_size);
    if (!result) {
        RC_HEAP_CALL(free_string, buffer, memory_resource);
        return create_invalid_text();
    }

    return wrap_text_with_length(buffer, buffer_size, true, memory_resource);
}
