...
typedef struct tag_TEXT (*func_split_text)(const TEXT* source, size_t* next_index, const MEMORY_ARENA_RESOURCE* memory_arena_resource);
...
#ifdef RES_CHECK
#   define add_text(source, text, memory_arena_resource) RC_HEAP_WRAP(add_text, source, text, memory_arena_resource)
#endif
...
#ifdef RES_CHECK
#   define join_text(separator, texts, count, ignore_empty, memory_arena_resource) RC_HEAP_WRAP(join_text, separator, texts, count, ignore_empty, memory_arena_resource)
#endif
...
#ifdef RES_CHECK
#   define trim_text(text, targets, characters, count, memory_arena_resource) RC_HEAP_WRAP(trim_text, text, targets, characters, count, memory_arena_resource)
#endif
...
#ifdef RES_CHECK
#   define add_text(source, text, memory_arena_resource) RC_HEAP_WRAP(add_text, source, text, memory_arena_resource)
#   define trim_whitespace_text(text, memory_arena_resource) RC_HEAP_WRAP(trim_whitespace_text, text, memory_arena_resource)
#endif
...
#ifdef RES_CHECK
#   define split_text(text, function, memory_arena_resource) RC_HEAP_WRAP(split_text, text, function, memory_arena_resource)
#endif
...
#ifdef RES_CHECK
#   define split_newline_text(text, memory_arena_resource) RC_HEAP_WRAP(split_newline_text, text, memory_arena_resource)
#endif
