...
TCHAR* RC_HEAP_FUNC(clone_string, const TCHAR* source)
{
    ...
    TCHAR* result = (TCHAR*)RC_HEAP_CALL(allocate_raw_memory, bytes, false, DEFAULT_MEMORY);
...
TCHAR* RC_HEAP_FUNC(allocate_string, size_t length)
{
    ...
    TCHAR* result = RC_HEAP_CALL(allocate_raw_memory, bytes, true, DEFAULT_MEMORY);
    return result;
}

void RC_HEAP_FUNC(free_string, const TCHAR* s)
{
    RC_HEAP_CALL(free_memory, (void*)s, DEFAULT_MEMORY);
}
