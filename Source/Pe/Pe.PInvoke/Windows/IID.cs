using System;

namespace ContentTypeTextNet.Pe.PInvoke.Windows
{
    partial class NativeMethods
    {
        public static Guid IID_IImageList = new Guid("{46EB5926-582E-4017-9FDF-E8998DAA0950}");
        public static readonly Guid IID_IShellItem = new Guid("{43826d1e-e718-42ee-bc55-a1e261c37bfe}");
        public static readonly Guid IID_IShellLink = new Guid("{000214F9-0000-0000-C000-000000000046}");
        public static readonly Guid SID_SExplorerBrowserFrame = new Guid("000214F1-0000-0000-C000-000000000046"); // SID_SExplorerBrowserFrame
    }
}
